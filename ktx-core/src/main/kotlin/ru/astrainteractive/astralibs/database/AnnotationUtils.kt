package ru.astrainteractive.astralibs.database
...
val Class<*>.primaryConstructor: Constructor<*>?
    get() = constructors.firstOrNull {
        it.parameters.any { it.annotations.firstNotNullOfOrNull { it as? ColumnInfo } != null }
    }
...
fun <T> AnnotationUtils.EntityInfo.Companion.create(
    ...
    }.subList(0, primaryConstructor.parameterCount)

    val list = primaryConstructor.parameters?.mapIndexedNotNull { i, it ->
...
inline val <reified T, V> KProperty1<T, V>.columnInfo: ColumnInfo?
    get() =
        AnnotationUtils.EntityInfo.create(T::class.java)?.columns?.firstOrNull { it.fieldName == name }?.columnInfo
